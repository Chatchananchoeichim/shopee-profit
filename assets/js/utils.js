...
async function exportProPDF() {
  ...
  try {
    ...
    const activeTabId = document.querySelector('.tab-content.active') ? document.querySelector('.tab-content.active').id : '';
    const isSummaryTab = activeTabId === 'tab-summary';
    const isResultTab = activeTabId === 'tab-result';

    doc.setFontSize(16);
    doc.text(isSummaryTab ? `รายงานสรุปรายสินค้า Shopee - Torque` : `รายงานผลประกอบการ Shopee - Torque`, 14, 15);
    doc.setFontSize(10);
    ...
    if (!isSummaryTab) {
      ...
      doc.autoTable({
        ...
        didParseCell: function(data) {
          ...
          const resultItem = exportResults[rowIndex];
    ...
    if (!isResultTab && exportSummary && exportSummary.length > 0) {
      if (!isSummaryTab) {
        doc.addPage(); // Only add a new page if we drew orders first (e.g. Dashboard tab)
        doc.setFontSize(16);
    ...
    doc.save(`Torque_${isSummaryTab ? 'Summary' : 'Orders'}_Report_${getFormattedDateStr()}.pdf`);
...
function exportFullExcel() {
  ...
  // 3. Ads Sheet
  if (!isSummaryTab && !isResultTab && state.adsData && state.adsData.length > 0) {
  ...
  XLSX.writeFile(wb, `Torque_${isSummaryTab ? 'Summary' : 'Orders'}_Report_${getFormattedDateStr()}.xlsx`);
}
